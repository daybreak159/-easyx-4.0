...
bool button(int m,int x, int y, int w, int h, const char* text)
{
		setfillcolor(m);
...
int gamestart()
{
	...
	while (true)
	{
		...
		BeginBatchDraw();
		if (button(WHITE,350, 120, 150, 35, "game start"))
		{
			cout << "gamestart";
			cleardevice();
			EndBatchDraw();
		...
		if (button(WHITE,350, 60, 150, 35, "introduction"))
		{
			cleardevice();
			EndBatchDraw();
			msg = { 0 };
...
int choose()
{
	...
	while (true)
	{
		peekmessage(&msg, EM_MOUSE);
		BeginBatchDraw();
		if (button(0x55FFFF,150, 180, 150, 35, "two"))
		{
		...
		if (button(0x0000AA,150, 60, 150, 35, "one"))
		{
			cleardevice();
			EndBatchDraw();
			msg = { 0 };
		...
		if (button(0xFFD700,150, 300, 150, 35, "three"))
		{
			cleardevice();
			EndBatchDraw();
			msg = { 0 };
...
int game2()
{
	...
	//在指定位置贴图  前面的(0,0)是坐标
	int s = 2060;
	mciSendString(_T("open music2.mp3 alias bkmusic"), NULL, 100, NULL);
	setbkcolor(WHITE); 
	setlinecolor(acolor);
	setlinestyle(0, 5);
	line(0, 430, 640, 430);
	mciSendString(_T("play bkmusic "), NULL, 0, NULL);//循环播放
		...
		std::thread k2(tab, s * 48, 4);
		std::thread k3(tab, s * 49, 2);
		std::thread k4(tab, s * 50, 3);
		std::thread o1(tab, s * 51, 2);
		std::thread o2(tab, s * 52, 4);
		std::thread o3(tab, s * 53, 1);
		std::thread o4(tab, s * 54, 3);
		std::thread p1(tab, s * 55, 2);
		std::thread p2(tab, s * 56, 3);
		std::thread p3(tab, s * 57, 2);
		std::thread p4(tab, s * 58, 3);
		std::thread q1(tab, s * 59, 1);
		std::thread q2(tab, s * 60, 2);
		std::thread q3(tab, s * 61, 3);
		std::thread q4(tab, s * 62, 4);
		std::thread r1(tab, s * 63, 4);
		std::thread r2(tab, s * 64, 2);
		std::thread r3(tab, s * 65, 3);
		std::thread r4(tab, s * 66, 2);
...
int game3()
{
	...
	//在指定位置贴图  前面的(0,0)是坐标
	int s = 2550;
	mciSendString(_T("open music3.mp3 alias bkmusic"), NULL, 100, NULL);
	setbkcolor(WHITE);
	mciSendString(_T("play bkmusic "), NULL, 0, NULL);//循环播放
	std::thread t1(tab, 0, 1);
	std::thread t2(tab, s, 2);
